...
- Reference implementation: `@golden/certification` (CLI: `golden-certify check`) executes contract checks and emits failures with capability IDs.

### 2.1.1 Schema Drift Detection

The CI runner MUST compare proposed Input/Output schemas against the schema baseline to detect breaking changes.

- Baseline file: `packages/tools/certification/schema-baselines/capabilities.json` (generated via `golden-certify write-baseline`).
- Drift detection: `golden-certify check` fails the pipeline if any capability input/output or blueprint input schema changes are detected.
...
- Reference implementation: `golden-certify oscal` writes `dist/certification/oscal-component-definition.json`.
