/**
 * packages/blueprints/scripts/run-e2e.ts
 * Temporal e2e runner (clean Node process, not Vitest).
 *
 * Runs:
 * - bundle workflows
 * - start TestWorkflowEnvironment + Worker
 * - execute echoWorkflow
 * - assert runAs + traceId + GoldenContext trace_id reach the activity
 */
...
async function main() {
  if (process.env.ENABLE_DAGGER_E2E !== '1') {
    throw new Error(
      'Dagger e2e requires ENABLE_DAGGER_E2E=1. For propagation-only e2e, run: pnpm nx run blueprints:e2e'
    );
  }
