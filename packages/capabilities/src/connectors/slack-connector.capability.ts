...
> = {
    ...
    factory: (
        ...
        const payload = {
            ...
            botTokenRef: context.secretRefs.botToken,
        };
...
async function run() {
  // Read token from secret ref
  let token;
  if (input.botTokenRef && fs.existsSync(input.botTokenRef)) {
    token = fs.readFileSync(input.botTokenRef, "utf8").trim();
  } else {
