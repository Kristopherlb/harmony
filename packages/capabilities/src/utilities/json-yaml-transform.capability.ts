...
const inputSchema = z
    .object({
        ...
        deep: z.boolean().optional().default(true).describe('Perform deep merge (for merge operation)'),
        indent: z.number().int().min(0).max(8).optional().default(2).describe('Indentation spaces for output'),
        sortKeys: z.boolean().optional().default(false).describe('Sort object keys alphabetically'),
    })
...
const configSchema = z
    .object({
        defaultIndent: z.number().int().min(0).max(8).optional().default(2).describe('Default indentation'),
        defaultSortKeys: z.boolean().optional().default(false).describe('Default key sorting'),
    })
