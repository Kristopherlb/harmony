...
const CATALOG_ACTIONS: Action[] = [
  ...
  // ============================================
  // Phase 5: Sample runbooks (repo-local markdown)
  // IDs align with /runbooks/<id>.md so the Runbooks UI can link them.
  // ============================================
  {
    id: "api-health-check",
    name: "API Health Check",
    description: "Comprehensive API endpoint verification runbook",
    category: "remediation",
    riskLevel: "low",
    requiredParams: [
      { name: "service", type: "select", label: "Service", required: true, options: ["api", "gateway", "web"] },
      { name: "baseUrl", type: "url", label: "Base URL", required: true, placeholder: "https://api.example.com" },
    ],
    workflowId: "runbook-api-health-check",
    icon: "wrench",
    estimatedDuration: "2-5 min",
    requiredRoles: ["dev", "sre", "admin"],
    targetServices: ["api", "gateway", "web"],
    contextTypes: ["incident", "deployment_failure"],
  },
  {
    id: "redis-restart",
    ...
    requiredParams: [
      { name: "cluster", type: "select", label: "Redis Cluster", required: true, options: ["sessions", "api-cache", "feature-flags"] },
      { name: "verify", type: "boolean", label: "Verify health checks", required: false, placeholder: "Run health checks after restart" },
    ],
    workflowId: "runbook-redis-restart",
    icon: "database",
  ...
  },
  {
    ...
    description: "Reset application database connection pool safely",
    category: "remediation",
    riskLevel: "medium",
    requiredParams: [
      { name: "service", type: "select", label: "Service", required: true, options: ["api", "worker"] },
      { name: "environment", type: "select", label: "Environment", required: true, options: ["production", "staging"] },
    ],
    workflowId: "runbook-database-connection-pool",
    icon: "refresh-cw",
    estimatedDuration: "2-6 min",
    requiredRoles: ["sre", "admin"],
    targetServices: ["database", "postgres", "mysql", "api", "worker"],
    contextTypes: ["incident", "infrastructure"],
  },
  {
    id: "clear-cache",
    name: "Clear Cache",
    description: "Safe cache invalidation for a service or key pattern",
    category: "remediation",
    riskLevel: "medium",
    requiredParams: [
      { name: "cache", type: "select", label: "Cache", required: true, options: ["redis", "cdn", "app-cache"] },
      { name: "pattern", type: "string", label: "Key Pattern", required: false, placeholder: "user:*" },
    ],
    workflowId: "runbook-clear-cache",
    icon: "trash-2",
    estimatedDuration: "1-3 min",
    requiredRoles: ["sre", "admin"],
    targetServices: ["cache", "redis", "cdn"],
    contextTypes: ["incident", "infrastructure"],
  },
  {
    id: "rollback-deployment",
    name: "Rollback Deployment",
    description: "Kubernetes deployment rollback with verification",
    category: "deployment",
    riskLevel: "high",
    requiredParams: [
      { name: "namespace", type: "select", label: "Namespace", required: true, options: ["production", "staging"] },
      { name: "deployment", type: "string", label: "Deployment", required: true, placeholder: "api-server" },
      { name: "revision", type: "number", label: "Revision (optional)", required: false, placeholder: "0" },
    ],
    workflowId: "runbook-rollback-deployment",
    icon: "rocket",
    estimatedDuration: "5-12 min",
    requiredRoles: ["sre", "admin"],
    targetServices: ["kubernetes", "deployment", "api", "web", "gateway"],
    contextTypes: ["incident", "deployment_failure"],
  },
];
